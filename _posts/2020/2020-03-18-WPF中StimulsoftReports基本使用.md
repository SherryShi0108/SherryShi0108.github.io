...
2. 创建当前报表所需变量和数据源。字典-新建变量/新建数据源来进行创建，数据源选择从DataSet/DataTable中获取数据。增加栏（即字段），即可获得各列。![20200318_01](/_posts/2020/image/20200318_01.png)

3. 绘制report。在左边侧栏中选择想要的数据项进行绘制，注意数据需要在Band中存放否则会每页打印。

   + 文本：可以将文本放到数据区，单击属性即可选择相应的外观设置。双击文本框在表达式一栏中**手动**输入所对应的刚才创建的变量，用大括号包围。![20200318_02](/_posts/2020/image/20200318_02.png)

   + 图像：新建变量，选择类型为图像类型。绘制图像栏，双击弹出弹窗中输入数据一栏为刚才所建变量，即可绑定变量与图像。![20200318_03](/_posts/2020/image/20200318_03.png)
   + 数据表：添加DataBand数据区，双击该区域即可弹出数据源设置，选择所需的数据源。在其中插入文本框绑定数据表中的字段，即可在表中显示数据。![20200318_04](/_posts/2020/image/20200318_04.png)![20200318_05](/_posts/2020/image/20200318_05.png)
   + 其他：表头、多级标题等可用HeaderBand、FooterBand中添加文本框来完成。页脚也可以在所提供的Band中选择设置。![20200318_06](/_posts/2020/image/20200318_06.png)
